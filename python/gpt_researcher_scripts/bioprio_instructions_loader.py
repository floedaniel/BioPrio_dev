...
Usage:
    from bioprio_instructions_loader import load_instructions, build_justification_prompt

    # Load all instructions
    instructions = load_instructions()

    # Build prompt for a specific question
    prompt = build_justification_prompt('ENT1', 'Lasius neglectus')
"""
...
def load_instructions(
    ...
    if needs_regen:
        from parse_bioprio_instructions import parse_rmd_to_json
...
def build_justification_prompt(
    ...
    # Question header
    prompt_parts.append(f"""Answer the following question about {species_name}{pathway_text}:
...
def build_value_selection_prompt(
    ...
    if question_type == "minmax":
        prompt = f"""Select min/likely/max values for this species risk assessment question.
...
if __name__ == "__main__":
    # Test the loader
    print("Testing BioPRIO instructions loader...")
    print("=" * 60)
    ...
    try:
        value_prompt = build_value_selection_prompt(
            'ENT1',
            'Lasius neglectus',
            'The species has a wide distribution across Mediterranean regions and parts of Asia, covering approximately 5 million km².'
        )
