"""
BioPRIO Rmd Instructions Parser

Parses Instructions_BioPrio_assessments.rmd and generates structured JSON.

Adapted from FinnPRIO parser for terrestrial invertebrates:
- "pest" → "species"
...
- Handles format with ### Options and ### Guidance sections
...
Usage:
    python parse_bioprio_instructions.py
    python parse_bioprio_instructions.py --force
    python parse_bioprio_instructions.py path/to/Instructions.rmd --output path/to/output.json
"""
...
class RmdInstructionsParser:
    """Parser for BioPRIO Instructions Rmd file"""

    # Question code patterns - matches ## ENT1. or ## MAN1. or ## IMP4.1: etc.
    QUESTION_PATTERN = re.compile(
        r'^##\s+(?P<code>(?:ENT|EST|IMP|MAN)\d+[AB]?)(?:\.(?P<subnum>\d+))?[:\.]?\s*(?P<text>.+)$',
        re.MULTILINE
    )
        ...
        for match in self.QUESTION_PATTERN.finditer(self.content):
            ...
            if subnum:
                ...
                if code not in self.questions:
                    # Create a placeholder parent question with generic text
                    parent_text = f"Would the species have impacts in the risk assessment area? (see sub-questions)"
                    self.questions[code] = self._parse_question(code, parent_text, content)
                    self.questions[code]['sub_questions'] = []

                if 'sub_questions' not in self.questions[code]:
                    self.questions[code]['sub_questions'] = []

                # Avoid duplicate sub-question entries
                existing_codes = [sq['code'] for sq in self.questions[code]['sub_questions']]
                if sub_code not in existing_codes:
                    self.questions[code]['sub_questions'].append({
                        'code': sub_code,
                        'text': sub_text,
                ...
                # Update parent guidance with last sub-question's guidance if not set
                if not self.questions[code].get('guidance') and sub_guidance:
                    self.questions[code]['guidance'] = sub_guidance
            else:
    ...
    def _build_output(self) -> Dict:
        ...
        return {
            ...
            'risk_assessment_area': 'Generic (Norway-focused)',
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parse BioPRIO Rmd Instructions to JSON')
